...
// More initializers here.
{
  ...
  myTTree->Branch("shower_matched_MCP", "std::vector< bool >", &_shower_matched_MCP);
...
void lee::PandoraLEEAnalyzer::get_true_energy_dep_daughters(const std::vector<simb::MCParticle>& event_mcp, const simb::MCParticle& mcparticle, std::vector<double>& energy_dep) {
  ...
  for (Int_t i=0; i<3; i++)
    IntersectParams.push_back((Bounds[i+3*(int)ReverseOrder[i]]-ParticleEndPoints.at(i)) / (ParticleEndPoints.at(i+3)-ParticleEndPoints.at(i)));
  for (Int_t i=0; i<3; i++)
    IntersectParams.push_back((Bounds[i+3*(1-(int)ReverseOrder[i])]-ParticleEndPoints.at(i)) / (ParticleEndPoints.at(i+3)-ParticleEndPoints.at(i)));
 
  // Get min/max linear parameter between 0 and 1
  double ParticleParamStart = 0.0;
  double ParticleParamEnd = 1.0;
  for (Int_t i=0; i<3; i++) {
    if (IntersectParams.at(i) > ParticleParamStart)
      ParticleParamStart = IntersectParams.at(i);
    if (IntersectParams.at(i+3) < ParticleParamEnd)
      ParticleParamEnd = IntersectParams.at(i+3);
  }
 
  if (ParticleParamEnd < 0) ParticleParamEnd = 0;
  if (ParticleParamStart > 1) ParticleParamStart = 1; 
  if (ParticleParamEnd < ParticleParamStart) 
    ParticleParamEnd = ParticleParamStart;
  
  // Get vector of daughter MCParticles
  std::vector<simb::MCParticle> daughters;
  for (Int_t i_daught = 0; i_daught < mcparticle.NumberDaughters(); i_daught++)
    for (auto mcp : event_mcp)
      if (mcparticle.Daughter(i_daught) == mcp.TrackId())
        daughters.push_back(mcp);

  // If mcparticle is electron
  if (TMath::Abs(mcparticle.PdgCode()) == 11 || TMath::Abs(mcparticle.PdgCode()) == 2212) {
    double EnergyDeposited = mcparticle.E()-mcparticle.EndE();

    for (auto daughter : daughters)
      EnergyDeposited -= daughter.E() - daughter.EndE();
    ...
    for (auto daughter : daughters)
      get_true_energy_dep_daughters(event_mcp, daughter, energy_dep);
  }

  // If mcparticle is photon
  else if (mcparticle.PdgCode() == 22) {
    double EnergyDeposited = mcparticle.E(); // photon is massless, don't need to subract EndE()
  ...
  // If mcparticle is something else
  else {
    for (auto daughter : daughters)
      get_true_energy_dep_daughters(event_mcp, daughter, energy_dep);
  }
...
void lee::PandoraLEEAnalyzer::analyze(art::Event const &evt)
{
  ...
  if ((!evt.isRealData() || m_isOverlaidSample) && !m_isCosmicInTime)
  {
    ...
    if (generator.size() > 0)
    {
	  ...
	  NeutrinoID = gen.GetNeutrino().Nu().TrackId();
    ...
    auto const &event_mcp(*mcp_h);
   
    // Find the primary electron with the highest energy 
    if(there_is_a_neutrino)
    {
      int i_e = -1;
      double maxE = 0;
      for(unsigned int j = 0; j < event_mcp.size(); j++)
      {
	auto mcparticle = event_mcp.at(j);
        if(mcparticle.Mother() == NeutrinoID and mcparticle.Process() == "primary" and mcparticle.StatusCode() == 1 and mcparticle.PdgCode() == 11)
	{
	  if(mcparticle.E() > maxE)
	  {
	    maxE = mcparticle.E();
	    i_e = j;
	  }
      ...
      // If there is a primary electron, run true shower algorithm on corresponding mcparticle 
      if (i_e > -1)
      {
        std::vector<double> energy_dep = {0.0, 0.0};
        std::cout<<"[TrueShower] Running shower energy algorithm on particle with pdg code "<<event_mcp.at(i_e).PdgCode()<<" has index "<<i_e<<std::endl;
        get_true_energy_dep_daughters(event_mcp, event_mcp.at(i_e), energy_dep);
        std::cout<<"[TrueShower] Energy deposition: " <<energy_dep.at(0)<<std::endl;
        _true_shower_E_dep = energy_dep.at(0);
        _true_shower_E_dep_fiducial = energy_dep.at(1);
	elecTrackID = event_mcp.at(i_e).TrackId();
	std::cout<<"[TrueShower] Electron TrackID: "<<elecTrackID<<std::endl;	
      }
    }
  ...
  if (_event_passed)
  {
    ...
    for (auto &pf_id : _nu_shower_ids)
    {
      ...
      // Flag to see if shower is matched to the selected electron
      std::cout<<"[TrueShower] Shower ID: "<<shower_obj->ID()<<std::endl;
      if ( shower_obj->ID() == elecTrackID)
      	_shower_matched_MCP.push_back(true);
      else _shower_matched_MCP.push_back(false);
